...
test("uses the first supported browser language and falls back to Russian as the default", () => {
  assert.equal(resolveBrowserLocale(["ru-RU", "en-US"]), "ru");
  assert.equal(resolveBrowserLocale(["ru", "en-US"]), "ru");
  ...
  assert.equal(resolveBrowserLocale(["fr-FR"]), "ru");
  assert.equal(resolveBrowserLocale([]), "ru");
});

test("returns only registered locales", () => {
  assert.deepEqual(getSupportedLocales(), ["ru", "en", "zh-CN", "zh-TW"]);
  assert.equal(getLocalePlugin("ru").id, "ru");
