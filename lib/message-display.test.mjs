...
test("keeps text between process blocks visible in its own display segment", async () => {
  const { splitAssistantBlocksForDisplay } = await loadSubject();
  const message = assistant([
    { type: "thinking", thinking: "work through it" },
    { type: "toolCall", toolCallId: "call-1", toolName: "read", input: {} },
    { type: "text", text: "I found the relevant file." },
    { type: "toolCall", toolCallId: "call-2", toolName: "edit", input: {} },
    { type: "thinking", thinking: "verify the change" },
    { type: "text", text: "Done." },
  ]);

  const result = splitAssistantBlocksForDisplay(message);

  assert.deepEqual(
    result.map(({ kind, blocks, blockIndexOffset }) => ({
      kind,
      types: blocks.map((block) => block.type),
      blockIndexOffset,
    })),
    [
      { kind: "process", types: ["thinking", "toolCall"], blockIndexOffset: 0 },
      { kind: "content", types: ["text"], blockIndexOffset: 2 },
      { kind: "process", types: ["toolCall", "thinking"], blockIndexOffset: 3 },
      { kind: "content", types: ["text"], blockIndexOffset: 5 },
    ],
  );
});
